  ...
  class Characters < Controllers::Base
    ...
    declare_route 'get', '/:id/characters' do
      campaign = check_session_and_campaign(
        action: 'characters_list',
        strict: false
      )
      session = Arkaan::Authentication::Session.where(
    ...
    declare_route 'post', '/:id/characters' do
      ...
      invitation = campaign.invitations.where(
      ...
      if valid
        Services::Characters.instance.create(invitation, data)
