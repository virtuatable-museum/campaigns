RSpec.shared_examples 'DELETE /:id' do
  describe 'DELETE /:id' do
    ...
    describe 'Nominal case' do
      before do
        delete "/#{campaign.id}", {app_key: appli.key, session_id: session.token}
      end
    ...
    describe '400 errors' do
      describe 'session ID not given' do
        before do
          delete "/#{campaign.id}", {app_key: appli.key}
        end
    ...
    describe '403 error' do
      describe 'Session ID not allowed' do
        ...
        before do
          delete "/#{campaign.id}", {app_key: appli.key, session_id: another_session.token}
        end
    ...
    describe '404 errors' do
      describe 'session ID not found' do
        before do
          delete "/#{campaign.id}", {app_key: appli.key, session_id: 'unknown_session_id'}
        end
      ...
      describe 'Campaign not found error' do
        ...
        before do
          delete '/any_other_id', {app_key: appli.key, session_id: another_session.token}
        end
